...
export const PLANS = {
  free: { name: 'Free', dailyLimit: 50, priceRs: 0, features: { attachments: false, ccBcc: false }, displayFeatures: ['50 send credits every day', 'Basic email campaigns', 'Spreadsheet recipient import', '3-day trial'] },
  pro: { name: 'Pro', dailyLimit: 450, priceRs: 2500, features: { attachments: true, ccBcc: true }, displayFeatures: ['450 send credits every day', 'File attachments', 'CC / BCC support', 'Priority upgrade access', 'Share quota across up to 2 email accounts'] },
  business: { name: 'Business', dailyLimit: 1000, priceRs: 6000, features: { attachments: true, ccBcc: true }, displayFeatures: ['1,000 send credits every day', 'File attachments', 'CC / BCC support', 'Priority upgrade access', 'Share quota across up to 3 email accounts'] },
};
export const LEGACY_DAILY_LIMIT = parseInt(process.env.DAILY_SEND_LIMIT || '450', 10);
export const LEGACY_FEATURES = { attachments: true, ccBcc: true };
...
export function getRemainingQuota(email) {
...
export function recordSend(email) {
  const clients = readClients();
  const client = clients[email];
  if (!client) return;
